...
function [imgFace] = oneSideTransform(imgFace, Part, setting, ratio, deny)
   ...
   if setting_val >= 0
    ...
    for i = 1 : (Part(1,3)+1) * (Part(1,4)+1)
        ...
        for a = -r:r
            for b = -r:r
                ...
                temp(1) = temp(1) + uint32(imgFace(pos(1)+a, pos(2)+b, 1));%/(4*r*r+4*r+1);
                temp(2) = temp(2) + uint32(imgFace(pos(1)+a, pos(2)+b, 2));%/(4*r*r+4*r+1);
                temp(3) = temp(3) + uint32(imgFace(pos(1)+a, pos(2)+b, 3));%/(4*r*r+4*r+1);
                cnt  = cnt + 1;
            end
        end
    ...
    imgFace(Part(1,1),Part(1,2),:) = [255 0 0];
    %
%     
